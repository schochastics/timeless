...
is the powerhouse of the package and tries as hard as possible to parse
every input into either a date or a datetime, depending on `out_format`.
The function can also return a raw character vector which can be fed
into faster converters, such as
...
Under the hood
[`chronos()`](https://schochastics.github.io/chronos/reference/chronos.md)
calls three functions which can also be used in isolation:

- [`parse_datetime()`](https://schochastics.github.io/chronos/reference/parse_datetime.md):
  a fast datetime parser that tries several different formats until it
  can parse the input

- [`parse_date()`](https://schochastics.github.io/chronos/reference/parse_date.md):
  a fast date parser that tries several different formats until it can
  parse the input
